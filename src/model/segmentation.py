...
class FPAv2(nn.Module):
    ...
    def forward(self, x):
        ...
        x_glob = F.upsample(x_glob, scale_factor=16,
                            mode='bilinear', align_corners=True)  # 256, 16, 16
        ...
        d3 = F.upsample(d3, scale_factor=2, mode='bilinear',
                        align_corners=True)  # 256, 8, 8
        d2 = d2 + d3

        d2 = F.upsample(d2, scale_factor=2, mode='bilinear',
...
class GAB(nn.Module):
    def __init__(self, input_dim, reduction=4):
        ...
        self.conv2 = nn.Conv2d(input_dim // reduction,
                               input_dim, kernel_size=1, stride=1)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, e=None):
        x = F.upsample(input=x, scale_factor=2,
...
class Res34Unetv3(nn.Module):
    ...
    def forward(self, x):
        ...
        e = F.adaptive_avg_pool2d(
            e5, output_size=1).view(batch_size, -1)  # 512
        e = self.dropout(e)
...
class Res34Unetv5(nn.Module):
    ...
    def forward(self, x):
        ...
        f = torch.cat((d2,
                       F.upsample(d3, scale_factor=2,
                                  mode='bilinear', align_corners=True),
                       F.upsample(d4, scale_factor=4,
                                  mode='bilinear', align_corners=True),
